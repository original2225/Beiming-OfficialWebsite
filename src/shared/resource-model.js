export const fallbackNodes = [
  { id: 'amd-9950x', name: 'AMD-9950X', address: 'localhost' },
  { id: 'amd-7840h', name: 'AMD-7840h', address: '192.168.1.8' },
];

export function formatNodeDisplayName(node) {
  const rawName = String(node?.name || '').trim();
  const nodeId = String(node?.id || '').trim();
  if (!rawName) return nodeId || '-';
  let daemonHost = '';
  try {
    daemonHost = node?.daemonUrl ? new URL(node.daemonUrl).hostname : '';
  } catch {
    daemonHost = '';
  }
  const suffixes = [
  ...
  return suffixes.includes(suffix.trim().toLowerCase()) ? label.trim() : rawName;
}
...
export function mapContainersToResources(items, node) {
  const nodeName = formatNodeDisplayName(node);
  return (items || []).map((item) => ({
    ...
    load: item.stats?.cpuUsagePercent ?? item.stats?.cpuPercent ?? 0,
    owner: '节点',
