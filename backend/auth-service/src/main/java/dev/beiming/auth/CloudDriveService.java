...
public class CloudDriveService {
  ...
  private void initializeSchema() {
    ...
    """);
    jdbc.execute("create index if not exists idx_beiming_cloud_drives_user_id on beiming_cloud_drives(user_id)");
    jdbc.execute("alter table beiming_cloud_drives add column if not exists auth_mode varchar(40) not null default 'beiming'");
    jdbc.execute("""
    ...
    """);
    jdbc.execute("create index if not exists idx_beiming_cloud_oauth_states_user_id on beiming_cloud_oauth_states(user_id)");
    jdbc.execute("""
    ...
    """);
    jdbc.execute("create index if not exists idx_beiming_cloud_oauth_configs_user_id on beiming_cloud_oauth_configs(user_id)");
    jdbc.execute("alter table beiming_cloud_oauth_configs add column if not exists cdn_host text not null default ''");
  }
